...
export default function orders(state = initialState, action) {
  switch (action.type) {
    ...
    case 'DELETE_ORDERS':
      action.datafix.splice(
        action.datafix.findIndex(e => e.menuId == action.payload.menuId),
        1,
      );
    ...
    case 'UPDATE_ORDERS':
      let dataOrders = action.datapatch.findIndex(
        y => y.menuId == action.payload.menuId,
      );
    ...
    case 'INCREMENT':
      let dataincrement = action.datapatch.findIndex(
        y => y.menuId == action.payload.menuId,
      );
      ...
      action.datapatch = action.datapatch.splice(
        dataincrement,
        1,
        dataincrementfix,
      );
    ...
    case 'DECREMENT':
      let datadecrement = action.datapatch.findIndex(
        y => y.menuId == action.payload.menuId,
      );
      console.log(action.datapatch[datadecrement]);
      let qtyNewDec = action.payload.qty - 1;
      let datadecrementfix = {
        ...action.datapatch[datadecrement],
        qty: qtyNewDec,
      };
      // let datafixDrink = { ...action.drink[dataDrink], selected: true }
      // console.warn(datafixDrink)
